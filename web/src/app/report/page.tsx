...
export default function ReportPage() {
  ...
  return (
    <RequireAuth>
      <div className="relative min-h-screen bg-surface dark:bg-[#0D0D10]">
        ...
        <div className="container-px max-w-5xl py-10 sm:py-16 space-y-6 sm:space-y-8">
          ...
          >
            ...
            <h1 className="heading text-2xl sm:text-4xl font-extrabold tracking-tight text-ink dark:text-white">
              {stored.test.title}
            </h1>
            <p className="text-xs sm:text-sm text-black/50 dark:text-white/45">
          ...
          <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
            ...
            >
              ...
              <div className="grid w-full grid-cols-2 gap-2.5">
                <div className="rounded-lg border border-black/[0.06] bg-black/[0.015] p-3 text-center dark:border-white/[0.06] dark:bg-white/[0.02]">
                ...
                <div className="col-span-2 rounded-lg border border-black/[0.06] bg-black/[0.015] p-3 text-center dark:border-white/[0.06] dark:bg-white/[0.02]">
            ...
            <div className="flex flex-col gap-6">
              ...
              >
                <h2 className="heading mb-5 flex items-center gap-2 text-sm sm:text-base font-bold text-ink dark:text-white">
                  <ListChecks size={15} className="text-crimson" /> Topic breakdown
                </h2>
                    ...
                    <div key={`${t.subject}-${t.topic}`} className="space-y-1">
                      <div className="flex items-baseline justify-between gap-2 text-[12.5px]">
                        <span className="font-semibold text-ink/80 dark:text-white/80">
                          {t.topic}
                          <span className="ml-2 text-xs font-normal text-black/40 dark:text-white/40">{t.subject}</span>
                        </span>
                      ...
                      <div className="progress">
                        <motion.div
                          ...
                          className="progress-bar bg-crimson/70"
                        />
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <motion.div
                  ...
                  className="card border-crimson/10 p-5"
                >
                  <h3 className="heading flex items-center gap-1.5 text-[10.5px] uppercase tracking-wider text-crimson font-bold">
                    <Brain size={13} /> Weak areas
                  </h3>
                  <div className="mt-3 space-y-2">
                    {result.weak_areas.length === 0 && (
                      <p className="text-xs text-black/40 dark:text-white/40">No weak areas detected. Outstanding performance.</p>
                    )}
                    {result.weak_areas.map((w) => (
                      <div
                        key={w.topic}
                        className="flex items-center justify-between gap-2 rounded-lg bg-crimson-soft/50 border border-crimson/10 px-3 py-2 text-xs font-semibold text-crimson dark:bg-crimson/10 dark:text-crimson-light"
                      >
                ...
                <motion.div
                  ...
                  className="card border-success/10 p-5"
                >
                  <h3 className="heading flex items-center gap-1.5 text-[10.5px] uppercase tracking-wider text-success font-bold">
                    <Target size={13} /> Strong areas
                  </h3>
                  <div className="mt-3 space-y-2">
                    {result.strong_areas.length === 0 && (
                      <p className="text-xs text-black/45 dark:text-white/45">Practice more to record mastery milestones here.</p>
                    )}
                    {result.strong_areas.map((w) => (
                      <div
                        key={w.topic}
                        className="flex items-center justify-between gap-2 rounded-lg bg-success-soft border border-success/10 px-3 py-2 text-xs font-semibold text-success dark:bg-success/10 dark:text-green-400"
                      >
                        <span className="truncate">{w.topic}</span>
                        <span className="tabular-nums shrink-0">{formatMasteryPercent(w.accuracy)}%</span>
                      </div>
            ...
            <div className="p-6 sm:p-8 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[9.5px] font-semibold text-black/40 dark:text-white/40 uppercase tracking-wider">Evaluation insight</div>
                </div>
              </div>
          ...
          >
            <h2 className="heading mb-1 flex items-center gap-2 text-sm sm:text-base font-bold text-ink dark:text-white">
              <TrendingUp size={15} className="text-crimson" /> 7-day focus plan
            </h2>
            ...
            <div className="space-y-2.5">
              ...
              {plan?.map((p, i) => (
                <motion.div
                  ...
                  className="rounded-lg border border-black/[0.06] bg-black/[0.01] p-4 dark:border-white/[0.06] dark:bg-white/[0.01]"
                >
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="chip-crimson font-bold !py-0.5">
                      Day {i + 1}
                    </span>
                  ...
                  <p className="mt-1.5 text-[9px] font-semibold text-black/40 dark:text-white/40 uppercase tracking-wider">{p.reason}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
